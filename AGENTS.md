...
- Logo uses both image and gradient text; mobile hides text logo on scroll, desktop keeps both; logo text fully visible.  
- Hero CTAs full-width on small mobile.  
- Calendly modal contains minimal extra text.  
...
- Mid-tier labeled as most popular.  
- Top-tier labeled as custom pricing and visually treated as a normal price.  
...
- Desktop: navbar links/CTA initially hidden, revealed via animated focus transition aligned with hero CTAs, with distinct reveals.  
...
- Robots: index, follow.  
- Open Graph: title matches page title; description matches meta; image uses dedicated OG image; URL uses canonical.  
...
- Main H1 preserves readable whitespace in text content even when split across multiple spans.